...
# corresponding to Chris's notebook 5B
...
def perform_solve(solver, X, Y, correlation):
...
def notebook5b_part(n):
    print(
        "********* start step 5B part %s: ********** FittingCorrelationSpectra ***" % n
    )
    t0 = time.time()
    ...
    allow_neg = config["correlation_spectra"]["allow_negatives_in_hunt"]

    FIT_FOLDER = dirs.correlation_spectra_folder
    print(FIT_FOLDER)
    ...
    for subfolder in subfolders:
        # if 'Fig' in subfolder.name: continue
    ...
    t = int(time.time() - t0)
    print("\n****************************************** ")
    print("**  Step 5B part %s done, took %.2f min ***" % (n, t / 60))
    return t
